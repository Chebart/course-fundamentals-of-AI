...
from ..blocks import GRUBlock, Linear
...
class GRU(AbstractModel):
    def __init__(
        ...
        for i in range(num_layers):
            if i == 0:
                self.layers.append(GRUBlock(input_size, hidden_size))
            else:
                self.layers.append(GRUBlock(hidden_size, hidden_size))
